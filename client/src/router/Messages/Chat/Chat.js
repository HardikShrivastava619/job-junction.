...
export const chatLogic = () => {
  try {
    ...
    const handleGetUnseenMsg = async () => {
      try {
        const res = await fetch(
          `http://localhost:1800/api/message/getUnseenMsg/${loginData?.id}`,
        );
    ...
    const arrOfChat = finalChatterArr?.filter(
      (c, i) =>
        (c?.sender_id === loginData?.id ||
          c?.sender_id === currChatingUser?.id) &&
        (c?.reciever_id === loginData?.id ||
          c?.reciever_id === currChatingUser?.id),
    );

    const getALLMessengers = async () => {
      try {
        const res = await fetch(
          `http://localhost:1800/api/message/chatusers/${loginData?.id}`,
        );
        const data = await res.json();
    ...
    const uniqueById = finalChatterArr?.filter(
    ...
    const sendMsg = async () => {
      try {
        ...
        const res = await fetch(
          `http://localhost:1800/api/message/sendMsg/${loginData?.id}/${currChatingUser?.id}`,
          {
            method: "POST",
            body: msgData,
          },
    ...
    const updateSeenUnseenMSg = async () => {
      try {
        if (currChatingUser != null) {
          const res = await fetch(
            `http://localhost:1800/api/message/updateSeenMsg/${loginData?.id}/${currChatingUser?.id}`,
            {
              method: "PUT",
              headers: {
          ...
          if (data?.success) {
            socket.emit("seen-later", {
              senderId: currChatingUser?.id,
        ...
        if (deleteMSgId?.length) {
          await Promise.all(
            deleteMSgId?.map(async (m) => {
              const res = await fetch(
                `http://localhost:1800/api/message/deleteMsg/${m}`,
                {
                  method: "DELETE",
                },
              );
    ...
    const handleRemoveUnseenMsgFormSideBar = async () => {
      try {
        const filterUnssen = await unSeenMsg?.filter(
          (o) => o?.sender_id != currChatingUser?.id,
        );
