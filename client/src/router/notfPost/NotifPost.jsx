...
const NotifPost = () => {
  ...
  const handleLikers = async (i) => {
    if (!posts || ![...posts].reverse()[i] || ![...posts].reverse()[i].likes)
      return;
      ...
      const responses = await Promise.all(
        likerIds.map(async (m) => {
          const res = await fetch(
            `${import.meta.env.VITE_API_URL}/api/user/get_ourProfile/${m}`,
          );
      ...
      {posts?.length > 0 ? (
        <>
          ...
          {[...posts].reverse().map((p, i) =>
            p?.id == params?.pid ? (
              <div key={i} class="card  card-myPost mb-3">
                <div class="dropdown PiDotsThreeVerticalBoldpost-page ">
                  <div
                    className="PiDotsThreeVerticalBoldIcon-postpage  "
                ...
                <div
                  ...
                  class=" crouser-imgSlide  carousel slide"
                >
                  ...
                  <div style={{ height: "105%" }} class="carousel-inner">
                    {JSON.parse(p?.photo_uploads || "[]").map(
                      (imgPath, index) => (
                        <div
                          key={index}
                ...
                <div class="card-body notf-descrip-and-likecontai">
                  ...
                  <div className="notf-like-dilikecontt ">
                    {" "}
                    <div className=" mypost-likedislikecont">
                      <div>
                        {JSON.parse(p?.likes)?.includes(
                          ...
                          <SlLike
                            ...
                            onClick={() => {
                              hanldepostLike({ pid: p.id, id: loginData?.id });
                            }}
                          />
                        ) : (
                          <BiSolidLike
