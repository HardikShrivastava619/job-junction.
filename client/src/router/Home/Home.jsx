...
const {handleGetAllPosts, likedModal,hanldedeleteComment, commText,setText,allComm,hanldeGetComment,getFollowers,posts,following,getDateDifference,loginData,hanldepostDisLike,hanldepostLike,hanldeComment  } = HomeLogic()


const filteredPosts = posts?.filter(post=> following?.some(follower => follower?.followed_id  === post?.user_id)  || loginData?.id === post?.user_id     );
...
const handleLikers = async (i) => {
  ...
  if (!filteredPosts || ![...filteredPosts].reverse()[i] || ![...filteredPosts].reverse()[i].likes) return;
...
{ !likersModal ? <></> :<LikersModel  likedModal={likedModal}  cuurPost={cuurPost} recvid={recvid}  hanldepostDisLike={hanldepostDisLike} hanldepostLike={hanldepostLike}  setLikersModal={setLikersModal} likers={likers}  ></LikersModel>  }
    {filteredPosts?.length == 0  ? <div style={{ backgroundColor:'white' ,width:'71.2vw',height:'90vh' ,display:'flex' ,justifyContent:'center',alignItems:'center' } } >
      
      <img src="public\images\makeFriends.png"   style={{height:'50%'}} alt="" />
      
      
      
       </div>    : [...filteredPosts]?.reverse()?.map((p,i)=> <div  key={i}  className='home-cont card'  > 
    
    <div id={`carouselExampleIndicators${i}`}   class="carousel-home  carousel slide">
...
<Link className='post-user-info' to={`/OtherUserPosts/${p?.user_id}`} > { p?.profile_photo === null ?  <MdAccountCircle className='post-user-info-MdAccountCircle' />  : <img  className='post-user-home-img' src={p?.profile_photo?.split('..\\client\\')?.join('..\\..\\..\\')  } alt="" />  }     <small> {p?.name} </small>    </Link>
<div className='post-info'  >  

    <div  className='home-div-first '  > 
      <div   className=' home-likedislikecont '   >
<div  className='likeCont ' >  { JSON.parse(p?.likes)?.includes(loginData?.id?.toString()) === false 
|| p?.likes === null  ? <SlLike   style={{  cursor:'pointer',marginBottom:"1vh"}} 
 onClick={()=>{hanldepostLike( {pid:p.id,id:loginData?.id, rid:p?.user_id } )}}  /> 
  : <BiSolidLike   className='text-success' style={{cursor:'pointer',marginBottom:"1vh"}} 
   onClick={()=>{hanldepostLike({pid:p.id,id:loginData?.id,rid:p?.user_id })}} />  } 
   {p?.likes === null ? 0  : JSON.parse(p?.likes)?.length }     </div>
   
    <div>  
       {JSON.parse(p?.disLikes)?.includes(loginData?.id?.toString()) === false ||  p?.disLikes === null 
         ? <SlDislike className='mx-1'  style={{cursor:'pointer'}} 
         onClick={()=>{hanldepostDisLike({pid:p.id, id:loginData?.id,rid:p?.user_id })}}  />
          :  <BiSolidDislike className='text-danger'   style={{cursor:'pointer'}} onClick={()=>{hanldepostDisLike({pid:p.id, id:loginData?.id,rid:p?.user_id  })}}  ></BiSolidDislike> }  </div>  
...
<small style={{fontSize:'x-small'}} >  {convertUTCToIST(c?.created_at)}  </small>
    ...
    <div class="dropdown " >
    <div className='home-PiDotsThreeVerticalBold' data-bs-toggle="dropdown" aria-expanded="false" ><PiDotsThreeVerticalBold     />
    </div>
...
<small className='liker-name '  onClick={()=>{handleLikers(i); setLikersModal(true) ; setCurrPost(p?.id); setRecvid(p?.user_id)   }} >  show Likes..  </small>    
  
  

<small className='home-post-descrip' >  {p?.description}..  </small>    
<small className='home-post-timin text-secondary ' >  {getDateDifference(p?.created_at)?.days >= 1   ?  `${getDateDifference(p?.created_at)?.days} days ago `  :  getDateDifference(p?.created_at)?.hours >= 1 ?   `${getDateDifference(p?.created_at)?.hours} hours ago `   : getDateDifference(p?.created_at)?.minutes  >= 1  ? `${getDateDifference(p?.created_at)?.minutes} minutes ago `  : `${getDateDifference(p?.created_at)?.seconds} sec ago ` }   </small>
