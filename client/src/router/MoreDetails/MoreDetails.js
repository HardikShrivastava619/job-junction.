...
const deleteLang = async (lid) => {
    try {
        ...
        const res = await fetch(`http://localhost:1800/api/lang/deleteLang/${lid}`,{
            method:'DELETE'
        })
const data = await res.json();
...
const deleteSkills = async (sid) => {
    try {
        ...
        const res = await fetch(`http://localhost:1800/api/skills/deleteSkills/${sid}`,{
            method:'DELETE'
        })
const data = await res.json()
...
const getLang = async () => {
    try {
        const res = await fetch(`http://localhost:1800/api/lang/getLang/${params?.id}`)
const data = await res.json()
...
const getSkills = async () => {
    try {
        const res = await fetch(`http://localhost:1800/api/skills/getSkills/${params?.id}`)
const data = await res.json()
...
educFormData.append('certificate',certificate)
    ...
    const res = await fetch(`http://localhost:1800/api/education/addEducDet/${loginData?.id}`,{
      method: 'POST',
      body: educFormData,
    });
...
const handleDeleteeducdet = async (id) => {
    try {
        ...
        const res = await fetch(`http://localhost:1800/api/education/deleteEducDet/${id}`,{
            method:'DELETE'
        })
...
const getEducDet = async (e) => {
    ...
    const res = await fetch(`http://localhost:1800/api/education/getEducDet/${params?.id}`);
    const data = await res.json()
...
const handleSaveLang = async (e) => {
 ...
 e.preventDefault()
        ...
        const res = await fetch(`http://localhost:1800/api/lang/saveLang/${loginData?.id}`,{
            method:'POST',
            headers:{
...
const handleSaveSkills = async (e) => {
 ...
 e.preventDefault()
        ...
        const res = await fetch(`http://localhost:1800/api/skills/saveSkills/${loginData?.id}`,{
            method:'POST',
            headers:{
                'Content-Type':'application/json'
            },
...
const checkFollows = async () => {
    try {
        ...
        const res = await fetch(`http://localhost:1800/api/follow/checkFollowsOrnot/${followers_id}/${followed_id}`) 
const data = await res.json()
...
const getFollowers = async () => {
    try {
        ...
        const res = await fetch(`http://localhost:1800/api/follow/getAllFollowers/${followed_id}`) 
const data = await res.json()
...
const getFollowings = async () => {
    try {
        ...
        const res = await fetch(`http://localhost:1800/api/follow/getAllFollowings/${followed_id}`) 
const data = await res.json()
...
const notf =await handleCreateNotification({sid:loginData?.id ,rid:params?.id,eid:null ,type:'follow',action:data?.code })
...
///////////////////////////////////////////////////////// Enterprise Logic /////////////////////////////////////////////////////////////////////
 ...
 const  updateViewers = async () => {
  try {
        ...
        const res = await fetch(`http://localhost:1800/api/user/updateViewers/${params?.id}/${loginData?.id} `,{
          method :'PUT',
 headers:{
'Content-Type' : 'application/json'
},
...
if (data?.message == 'Added in view List') {
    const notf =await handleCreateNotification({sid:loginData?.id ,rid:params?.id,eid:null,type:'views' })
...
return {  title,setEmp,updateViewers, role, setNetWorth,   setBranches,   cover_image_url,sector,netWorth,chairman,chro,hq,designation,branches,totEmp,description,license_doc_url,official_site_url,

                                     /********************************** ENtp Returns are given above  **************************************** */ 
    
    
    showFollower,followings,getFollowings,setShowFollower,showFollowings,setShowFollowings,isFollow,followers,getFollowers,handleFollowBtn,checkFollows,profile_photo, params,loginData ,getLang,deleteLang,langText,handleSaveLang,langData,setLangData,setLangText,langForm,setLagForm, clickedStar,deleteSkills,setClickedStar,skillData,getSkills,handleSaveSkills,ratings,setRatings,skillText,setskillText,setSkillForm,skillForm,handleDeleteeducdet,pinCode,setPinCode,gender,fillEducationDet,getEducDet ,instituteName,setInstituteName,degree,setDegree,startDate,setStartDate,endDate,setEndDate,certificate,setCertificate,handleSaveChanges, handleEducationDetAddBtn,setFillEduc,educationDetails,edit,setEdit,editProf,setEditProf ,currStatus,expectedCTC ,handleEditModals, editNameModal,setEditNameModal,setExpectedCTC, setCurrStatus, setGender,handleMoreDetails,name,formattedDate,state,city,email,aboutMe,phnum,deg,company,post,landMark,insta,git,fb,twitter, }
